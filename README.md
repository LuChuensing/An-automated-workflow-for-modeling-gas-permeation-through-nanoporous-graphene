...
4. **Forcefield Mapping (Hybrid)**: Uses AIREBO for the internal stability of the graphene and edgd H. Applies Lorentz-Berthelot mixing rules to calculate precise Lennard-Jones (LJ) parameters for the non-bonded interactions between the gas molecule and the graphene/hydrogen atoms.  
5. **Z-Scan Simulation**: Generates and executes a LAMMPS script that moves the gas molecule through the pore, recording the interaction energy at each step to create a potential energy profile.  
